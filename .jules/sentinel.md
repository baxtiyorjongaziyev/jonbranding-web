...
## 2024-05-24 - Missing Input Validation on Error Reporting Endpoint
**Vulnerability:** The `/api/report-error` endpoint accepted unbounded input fields (`message`, `stack`, `pathname`, `userInfo`) from the client and forwarded them to Telegram.
**Learning:** Endpoints that accept arbitrary data from clients and forward it to third-party services (like Telegram) without length limits or validation can be abused to send massive payloads, leading to potential Denial of Service (DoS) or unexpected behavior in the downstream service.
**Prevention:** Always validate and enforce strict length limits on all client-provided data using schema validation libraries like `zod` before processing or forwarding it. Handle validation errors gracefully without leaking internal details.
