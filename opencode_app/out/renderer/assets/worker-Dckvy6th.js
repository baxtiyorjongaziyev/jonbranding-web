  ...
  function getContext(shiki) {
    if (!map.has(shiki.meta)) {
      let normalizePosition2 = function(p2) {
        if (typeof p2 === "number") {
          if (p2 < 0 || p2 > shiki.source.length) throw new ShikiError$2(`Invalid decoration offset: ${p2}. Code length: ${shiki.source.length}`);
          return {
          ...
          if (line === void 0) throw new ShikiError$2(`Invalid decoration position ${JSON.stringify(p2)}. Lines length: ${converter.lines.length}`);
          let character = p2.character;
          if (character < 0) character = line.length + character;
          if (character < 0 || character > line.length) throw new ShikiError$2(`Invalid decoration position ${JSON.stringify(p2)}. Line ${p2.line} length: ${line.length}`);
          return {
            ...p2,
            character,
...
function codeToTokens(primitive, code, options) {
  ...
  if ("themes" in options) {
    ...
    if (defaultColor && "light-dark()" !== defaultColor && !themes.some((t) => t.color === defaultColor)) throw new ShikiError$2(`\`themes\` option must contain the defaultColor key \`${defaultColor}\``);
...
var JavaScriptScanner = class {
  ...
  constructor(patterns, options = {}) {
    ...
    const { forgiving = false, cache: cache2, regexConstructor } = options;
...
function relativeLuminance(color) {
  ...
  const g = rgba[1] / 255;
  const b2 = rgba[2] / 255;
  const channel = (v2) => v2 <= 0.03928 ? v2 / 12.92 : ((v2 + 0.055) / 1.055) ** 2.4;
  return 0.2126 * channel(r2) + 0.7152 * channel(g) + 0.0722 * channel(b2);
}
...
function fill(colors, key2, value) {
  if (value != null && value !== "") colors[key2] = value;
}
...
function replaceCustomExtensions(version, map) {
  ...
  for (const key2 in map) {
    const lang = map[key2];
    if (lang != null) CUSTOM_EXTENSION_TO_FILE_FORMAT.set(key2, lang);
  }
...
//# sourceMappingURL=worker-Dckvy6th.js.map
