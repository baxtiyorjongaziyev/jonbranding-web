...
const ServiceCard = ({ id, onSelect, selected, lang, dictionary, currency }: { id: string, onSelect: () => void, selected: boolean, lang: any, dictionary: any, currency: any }) => {
    ...
    return (
        <Card
            onClick={onSelect}
            className={cn(
                "group relative h-full transition-all duration-300 cursor-pointer overflow-visible border-2 flex flex-col rounded-[1.2rem] mt-2",
                selected
        ...
        >
            <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 z-20">
                {recommended && !isVip && (
                    <Badge className="bg-primary text-white text-[8px] font-bold px-3 py-0.5 rounded-full border-none uppercase tracking-wider whitespace-nowrap shadow-md">
                        {dictionary.recommended}
                    </Badge>
                )}
                {isVip && (
                    <Badge className="bg-gradient-to-r from-amber-400 to-amber-600 text-blue-950 text-[8px] font-black px-4 py-0.5 rounded-full border-none uppercase flex items-center gap-1 shadow-md whitespace-nowrap">
                        VIP
                    </Badge>
                )}
            </div>
            ...
            <CardHeader className="p-3 pb-1 sm:p-4 sm:pb-1">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className="flex flex-col min-w-0">
                        <CardTitle className={cn("text-sm sm:text-base font-black leading-none tracking-tight truncate", isVip ? "text-white" : "text-dark-blue")}>
                            {label}
                        </CardTitle>
                        {subDescription && (
                            <span className={cn("text-[7px] font-bold uppercase tracking-wider mt-0.5", isVip ? "text-amber-400/60" : "text-primary/60")}>
                                {subDescription}
                            </span>
                        )}
                        <span className={cn("text-xs sm:text-sm font-black mt-0.5", isVip ? "text-amber-400" : "text-primary")}>
            ...
            <CardContent className="px-3 pt-0 pb-3 sm:px-4 sm:pt-0 sm:pb-4 flex-grow flex flex-col">
                <div className="flex border-b border-slate-100 mb-1.5" onClick={(e) => e.stopPropagation()}>
                    <button 
                        onClick={() => setActiveTab('included')}
                        className={cn(
                            "flex-1 py-1 text-[7px] font-black uppercase tracking-widest transition-all",
                            activeTab === 'included' 
                            ...
                            {(results || features || []).map((r: string, i: number) => (
                                <li key={i} className="flex items-start gap-1.5">
                                    <div className={cn("mt-0.5 shrink-0 rounded-full p-0.5", isVip ? "bg-amber-400/20" : "bg-primary/10")}>
                                        <CheckCircle className={cn("w-2.5 h-2.5", isVip ? "text-amber-400" : "text-primary")} />
                                    </div>
                                    <span className={cn("text-[10px] sm:text-[11px] font-normal leading-tight", isVip ? "text-slate-300" : "text-dark-blue")}>{r}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-1 flex-grow animate-fade-in">
                        {benefits && benefits.map((b: any, i: number) => (
                            <div 
                                key={i} 
                                className={cn(
                                    "p-1.5 rounded-lg border flex flex-col gap-0.5 transition-all duration-300",
                                    isVip ? "bg-white/5 border-white/10" : "bg-slate-50 border-slate-100"
                                )}
                            >
                                <span className="text-[10px]">{b.icon}</span>
                                <div className="space-y-0">
                                    <p className={cn("text-[9px] font-bold leading-tight", isVip ? "text-white" : "text-dark-blue")}>{b.title}</p>
                                    <p className={cn("text-[8px] leading-tight", isVip ? "text-slate-400" : "text-slate-500")}>{b.description}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="mt-2 pt-1.5 space-y-1.5">
                    ...
                    <Button
                        ...
                        className={cn(
                            "w-full py-2.5 text-[9px] font-bold transition-all duration-300 rounded-full border-2 h-auto uppercase tracking-wider",
                            selected 
                                ? (isVip 
                                    ? "border-none bg-gradient-to-br from-amber-400 to-amber-600 text-blue-950 hover:from-amber-500 shadow-md" 
...
const ServiceGroup = ({ title, children, gridCols = "lg:grid-cols-3" }: { title: string, children: React.ReactNode, gridCols?: string }) => (
    <div className="space-y-3 sm:space-y-4">
        <div className="flex items-center gap-3">
            <div className="h-5 sm:h-6 w-1 bg-primary rounded-full shadow-[0_0_8px_rgba(37,99,235,0.3)]" />
            <h3 className="text-base sm:text-lg font-black text-dark-blue tracking-tight uppercase">{title}</h3>
        </div>
        <div className={cn("grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4", gridCols)}>{children}</div>
    </div>
);
...
const PackageBuilder: FC<PackageBuilderProps> = ({ onOrderNow, lang, dictionary }) => {
    ...
    return (
        <section id="package-builder" className="py-8 sm:py-12 bg-white overflow-hidden" suppressHydrationWarning>
            <div className="container mx-auto px-4 max-w-7xl">
                <div className="max-w-4xl mx-auto mb-8 sm:mb-10 text-center space-y-1">
                    <Badge className="bg-primary/10 text-primary border-none px-4 py-1 rounded-full font-black text-[7px] uppercase tracking-[0.2em]">
                        LOYIHA ME'MORI
                    </Badge>
                    <h2 className="text-2xl sm:text-3xl font-black text-dark-blue leading-tight tracking-tighter">{translations.title}</h2>
                    <p className="text-xs sm:text-sm text-slate-500 max-w-2xl mx-auto font-medium">{translations.subtitle}</p>
                </div>
                ...
                <div className="space-y-8 sm:space-y-12">
                    ...
                    <Accordion type="single" collapsible className="w-full">
                        <AccordionItem value="more" className="border-none">
                            <AccordionTrigger className="text-sm font-black text-dark-blue justify-center gap-3 hover:no-underline py-4 sm:py-6 bg-slate-50 rounded-[1.2rem] border-2 border-dashed border-slate-200 transition-all hover:bg-slate-100 group">
                                {translations.categories.more_services}
                                <ChevronsDown className="w-3 h-3 text-primary animate-bounce" />
                            </AccordionTrigger>
                            <AccordionContent className="pt-6 sm:pt-8 space-y-8">
                ...
                <div className="mt-10 sm:mt-12 max-w-6xl mx-auto">
                    <div id="your-package-card" className="rounded-[1.5rem] bg-white shadow-2xl overflow-hidden flex flex-col lg:flex-row border border-slate-100">
                        <div className="lg:w-1/2 bg-dark-blue p-5 sm:p-8 text-white relative">
                        ...
                        <div className="lg:w-1/2 bg-slate-50 p-5 sm:p-8 flex flex-col border-l border-slate-100 relative">
