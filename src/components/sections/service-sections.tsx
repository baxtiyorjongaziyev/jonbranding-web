    ...
    <BrandSection tone="soft">
      <div className="container mx-auto px-4">
        <SectionIntro
          ...
          description={
            dictionary.subtitle ||
            dictionary.serviceSections?.subtitle ||
            'Logo, naming, strategiya va brandbook alohida emas - bitta biznes tizim sifatida ishlaydi.'
          }
        />
        <div className="mx-auto mt-8 grid max-w-4xl gap-3 sm:grid-cols-3">
          {servicePromise.map((item) => (
            <div
              key={item}
              className="flex items-center gap-2 rounded-full border border-brand-line bg-white/75 px-4 py-3 text-sm font-black text-brand-ink"
            >
            ...
            return (
              <motion.article key={service.id} variants={itemVariants}>
                <BrandCard className="group relative flex h-full flex-col overflow-hidden p-8">
                  ...
                  <h3 className="text-2xl font-black tracking-[-0.03em] text-brand-ink">
                    {service.title}
                  </h3>
                  <p className="mt-3 flex-grow text-base leading-7 text-brand-slate">
                    {service.description}
                  </p>
                  <Button
                    asChild
                    variant="ghost"
                    className="mt-6 justify-start px-0 text-brand-blue hover:bg-transparent hover:text-brand-ink"
                  >
