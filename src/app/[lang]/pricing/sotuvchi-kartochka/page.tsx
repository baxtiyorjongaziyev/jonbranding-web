...
const TierTable = ({
  ...
  <div
    className={`w-full mt-4 text-sm rounded-lg overflow-hidden border ${isRecommended ? 'border-blue-800' : 'border-slate-200 dark:border-slate-700'}`}
  >
    <div
      className={`grid grid-cols-2 text-center ${isRecommended ? 'bg-blue-900/70' : 'bg-slate-50 dark:bg-slate-800'}`}
    >
      <div
        className={`font-semibold p-2 border-b border-r ${isRecommended ? 'text-blue-100 border-blue-800' : 'text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700'}`}
      >
        Miqdor
      </div>
      <div
        className={`font-semibold p-2 border-b ${isRecommended ? 'text-blue-100 border-blue-800' : 'text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700'}`}
      >
        Narx (1 dona)
      </div>
    </div>
    <div className="grid grid-cols-2 text-center">
      {data.map((row, index) => (
        <React.Fragment key={index}>
          <div
            className={`p-2 border-r ${isRecommended ? 'border-blue-800' : 'border-slate-200 dark:border-slate-700'} ${index < data.length - 1 ? 'border-b' : ''} ${isRecommended ? 'text-blue-100' : 'text-slate-600 dark:text-slate-300'}`}
          >
            {row.quantity}
          </div>
          <div
            className={`p-2 ${index < data.length - 1 ? `border-b ${isRecommended ? 'border-blue-800' : 'border-slate-200 dark:border-slate-700'}` : ''} ${isRecommended ? 'text-blue-100' : 'text-slate-600 dark:text-slate-300'}`}
          >
            {row.price.toLocaleString('fr-FR')} so‘m
          </div>
        </React.Fragment>
      ))}
    </div>
...
const PricingCard = ({ tier }: { tier: (typeof pricingData)[0] }) => (
  <div
  ...
  >
    ...
    <h3
      className={`text-2xl font-bold text-center ${tier.isRecommended ? 'text-white' : 'text-slate-900'}`}
    >
      {tier.name}
    </h3>
    <p
    ...
    <div className="text-center my-6">
      <span className="text-5xl font-extrabold">{tier.price.toLocaleString('fr-FR')}</span>
      <span
    ...
    >
      ...
      <p className={`mt-2 text-sm ${tier.isRecommended ? 'text-blue-200' : 'text-slate-500'}`}>
        {tier.difference}
      </p>
    </div>
    <div
    ...
    <div className="mt-8">
      <a
        ...
        className="inline-flex items-center justify-center whitespace-nowrap rounded-full text-sm font-medium ring-offset-background transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98] bg-gradient-to-br from-dark-blue to-primary text-primary-foreground shadow-lg btn-animated-border hover:from-primary hover:to-dark-blue h-10 px-4 py-2 w-full"
      >
...
const ProductSchema = {
  ...
  description:
    'Sotuvlarni oshiradigan professional mahsulot kartochkasi dizayni. Marketplace uchun START, PRO va PREMIUM tariflar bilan tanishing va buyurtma bering.',
  brand: {
    '@type': 'Brand',
    name: 'Jon Branding',
  },
  ...
  return (
    <>
      ...
      <main className="bg-slate-50 dark:bg-slate-900 min-h-screen text-slate-800 dark:text-slate-200 pt-20">
        ...
        <section id="pricing-tiers" aria-labelledby="pricing-title" className="py-20">
              ...
              {pricingData.map((tier) => (
                ...
                >
                  ...
                  <div className="mt-4 flex justify-center">
                    <div
                      className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${selectedTier === tier.price ? 'border-blue-600 bg-blue-600' : 'border-slate-300'}`}
                    >
          ...
          <div className="container mx-auto px-6">
            <div className="max-w-2xl mx-auto bg-white dark:bg-slate-950 rounded-2xl shadow-lg p-8 relative overflow-hidden">
              ...
              <h2
                ...
                className="text-2xl font-bold text-center text-blue-950 dark:text-white"
              >
              ...
              <div className="mt-6 space-y-4">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <span className="font-medium text-slate-700 dark:text-slate-200">
                        1 ta mahsulot uchun 2 xil dizayn varianti (A/B test uchun)
                      </span>
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        Tanlangan paket narxiga +30% qo'shiladi
                      </p>
                    </div>
                  </div>
                ...
                <label className="flex justify-between items-center gap-4 cursor-pointer p-4 rounded-xl border border-slate-100 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-900 transition-colors">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <span className="font-medium text-slate-700 dark:text-slate-200">
                        24 soatda tezkor tayyorlash
                      </span>
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        Tanlangan paket narxiga +50% qo'shiladi
                      </p>
                    </div>
                  </div>
                  <span className="font-semibold text-slate-800 dark:text-white whitespace-nowrap">
                ...
                <label className="flex justify-between items-center gap-4 cursor-pointer p-4 rounded-xl border border-slate-100 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-900 transition-colors">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        Tanlangan paket narxiga +100% qo'shiladi
                      </p>
                    </div>
                  </div>
                  <span className="font-semibold text-slate-800 dark:text-white whitespace-nowrap">
              ...
              <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800">
                <div className="flex justify-between items-end">
                  ...
                  <Button
                    size="lg"
                    className="rounded-full bg-blue-600 hover:bg-blue-700 shadow-lg px-8"
                  >
                    Buyurtma berish
                  </Button>
                </div>
        ...
        <section id="faq" aria-labelledby="faq-title" className="pb-20">
          <div className="container mx-auto px-6">
            <div className="max-w-3xl mx-auto">
              <h2
                id="faq-title"
                className="text-3xl font-bold text-center text-blue-950 dark:text-white"
              >
              ...
              <div className="mt-8 space-y-4">
                <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-4">
                  <h3 className="font-semibold text-slate-800 dark:text-white">
                    Nega faqat sotuvchi kartochka rasmi dizayn qilamiz?
                  </h3>
                  <p className="text-slate-600 dark:text-slate-300 mt-2">
                    Chunki marketplace sotuvlarining 80% birinchi rasmga bog'liq. Biz aynan shu eng
                    muhim nuqtaga e'tibor qaratib, maksimal natija berishni maqsad qilganmiz.
                  </p>
                </div>
                <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-4">
                  <h3 className="font-semibold text-slate-800 dark:text-white">
                    Lightbox suratlar yetarlimi yoki studio kerakmi?
                  </h3>
                  <p className="text-slate-600 dark:text-slate-300 mt-2">
                    Start va Pro tariflar uchun lightbox suratlar yetarli. Premium tarifda esa
                    mahsulotni yanada jozibador ko'rsatish uchun studiyada olingan professional
                    suratlardan foydalanishni tavsiya etamiz.
                  </p>
                </div>
                <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-4">
                  <h3 className="font-semibold text-slate-800 dark:text-white">
                    Uzum/WB/Ozon o‘lchamlariga moslab berasizmi?
                  </h3>
                  <p className="text-slate-600 dark:text-slate-300 mt-2">
                    Albatta. Biz har bir marketplace'ning texnik talablarini bilamiz va dizaynni
                    aynan kerakli o'lcham va formatda tayyorlab beramiz.
                  </p>
                </div>
                <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-4">
                  <h3 className="font-semibold text-slate-800 dark:text-white">
                  ...
                  <p className="text-slate-600 dark:text-slate-300 mt-2">
                    Agar buyurtma 1 dona bo'lsa, to'lov 100% oldindan amalga oshiriladi. Agar
                    buyurtmalar soni bir nechta bo'lsa, 50% oldindan to'lov va qolgan 50% ish
                    topshirilgandan so'ng to'lanadi. Bu bizga sizning loyihangizni darhol boshlashga
                    va unga kerakli vaqt va resurslarni ajratishga imkon beradi.
                  </p>
                </div>
              </div>
            </div>
