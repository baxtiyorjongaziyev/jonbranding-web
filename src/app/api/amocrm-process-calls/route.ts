...
async function handleCallProcessing(request: Request) {
  try {
    ...
    for (const lead of leads) {
      ...
      // First, fetch the lead notes to check if it has already been processed/summarized
      ...
      // Look for call recordings on the lead's contacts
      ...
      // Process only the latest call recording associated with this lead to prevent spam
      const latestCall = callNotes[callNotes.length - 1];

      try {
        // Download audio file to memory buffer
        ...
        // Analyze audio with Gemini 1.5 Flash
        ...
        // 2. Patch lead to add category tag
    ...
    return NextResponse.json({ error: error.message || 'Fatal server error' }, { status: 500 });
  }
}
