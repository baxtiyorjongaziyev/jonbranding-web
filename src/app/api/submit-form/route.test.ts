...
describe('POST /api/submit-form', () => {
  ...
  it('prefers stable serviceKeys over localized packageSummary text for attribution', async () => {
    const storeMock = await import('@/lib/affiliate/store');
    vi.spyOn(storeMock, 'findAffiliateByPromoCode').mockResolvedValue({
      id: 'aff-1',
      promoCode: 'SHERBEK',
    ...
    const response = await POST(new Request('http://localhost/api/submit-form', {
      ...
      body: JSON.stringify({
        ...
        phone: '90 123 45 67',
        promoCode: 'sherbek',
        // Localized text that would NOT tokenize to a known service on its own.
        packageSummary: 'Қадоқ дизайни',
        // Stable calculator IDs — must win over packageSummary.
        serviceKeys: ['packaging'],
        source: 'brand_audit_offer',
        lang: 'uz',
      }),
    }));
